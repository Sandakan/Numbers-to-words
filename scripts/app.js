...
const threeParts = (inputString) => {
    let inputParts = [];
    let noOfParts = inputString.length % 3 !== 0
        ? Math.floor(inputString.length / 3) + 1
        : Math.floor(inputString.length / 3);
    for (let x = 0; x < noOfParts; x++) {
        inputParts.push(inputString.substring(inputString.length - 3 * (1 + x), inputString.length - 3 * x));
    }
    return {
...
const main = (parts) => {
    ...
    parts.length === 1 && parts[0] === '0'
        ...
        : parts
            .reverse()
            .forEach((part, currentPartNumber) => convertedParts.push(`${currentPartNumber !== parts.length - 1 ? namesOfExtras[currentPartNumber] : ''}${convert(part)}`));
...
const convert = (part) => {
    if (part.length === 1)
        return ones(part);
    if (part.length === 2)
        return `${tens(part)} ${ones(part)}`;
    if (part.length === 3)
        return `${hundreds(part)} ${tens(part)} ${ones(part)}`;
};
const ones = (part) => {
    if (part[part.length - 2] !== '1')
        return namesOfUnits[Number(part[part.length - 1])];
    else
        return '';
};
const tens = (part) => {
    if (part[part.length - 2] === '1')
        return namesOfSpecials[Number(part[part.length - 1]) - 1];
    else
        return namesOfTens[Number(part[part.length - 2])];
};
const hundreds = (part) => {
...
document.getElementById('btn').addEventListener('click', () => {
    ...
    (document.getElementById('result-container')).innerHTML = `<div>parts : ${parts}</div>
